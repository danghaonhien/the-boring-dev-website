...
const Pitch02ProjectPage: React.FC = () => { // Renamed component
  ...
  const createCards = useCallback((variant: 'propositions') => {
    ...
    const GraphStatsCard: React.FC<{ isDark?: boolean, data?: number[] }> = ({ isDark = false, data = [30, 60, 45, 75, 50] }) => { // Added more data points
        const maxValue = 100; // Assume percentage-based height
        // Define gradients for bars
        const barGradientFrom = 'from-gray-300';
        const barGradientTo = 'to-gray-400';
        const highlightGradientFrom = 'from-blue-500';
        const highlightGradientTo = 'to-blue-600';

        return (
            <CardPreview isDark={false} className={`flex flex-col p-4 bg-white border border-gray-200`}>
              <div className={`text-sm font-semibold mb-4 text-gray-800`}>Performance Snapshot</div> {/* Larger title */}
              ...
              <div className="flex-grow flex items-end justify-center space-x-3 h-32 px-2"> 
               ...
               <div className={`mt-3 text-xs text-center text-gray-400`}>Key Performance Indicators</div>
            </CardPreview>
        );
    };

    // Phases Timeline Card (Using Linear Horizontal Timeline)
    const PhasesTimelineCard: React.FC<{ isDark?: boolean }> = ({ isDark = true }) => {
      const phases = [
      ...
      return (
        <CardPreview isDark={false} className={`col-span-2 md:col-span-3 p-6 bg-white`}>
             <div className={`text-xxs font-medium text-gray-400 mb-1 uppercase tracking-wider`}>Project Timeline</div>
             <h3 className={`text-2xl font-bold mb-1 text-black`}>Phases</h3>
             <p className={`text-sm text-gray-600 mb-4`}>High-Level Stages</p>
             <p className={`text-xs text-gray-700 mb-10 max-w-md`}>A visual overview of the project's key phases, outlining the progression from initiation to completion and expected milestones.</p>
            ...
            <div className="relative w-full h-16 mb-4">
                ...
                <div className="relative flex justify-between h-full">
                    ...
                    <div
                        className={`absolute top-1/2 left-0 h-0.5 bg-gray-600 -translate-y-1/2`}
                    ...
                    {[...Array(numPhases + 1)].map((_, index) => (
                        <div
                            key={`marker-${index}`}
                            className={`absolute top-1/2 -translate-y-1/2 -translate-x-1/2 w-3 h-3 rounded-full border-2 bg-white border-gray-500`}
                            style={{ left: `${(index / numPhases) * 100}%` }}
                        >
                            {/* Highlight first marker? */}
                            {index === 0 && <div className={`w-full h-full rounded-full bg-gray-600`}></div>}
                        </div>
                    ))}
                </div>
             </div>
             ...
             <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-xs">
                 {phases.map((phase, n) => (
                    <div key={n} className={`text-gray-600 text-left pl-1`}>
                        <div className={`font-semibold mb-1 text-gray-800`}>{`0${n + 1} ${phase.title}`}</div>
    ...
    const QuarterlyGridCard: React.FC<{ isDark?: boolean }> = ({ isDark = false }) => {
       ...
       return (
        <CardPreview isDark={false} className={`col-span-2 md:col-span-3 p-6 bg-gray-100`}>
             <div className={`text-xxs font-medium text-gray-500 mb-1 uppercase tracking-wider`}>Quarterly Plan</div>
             <h3 className={`text-2xl font-bold mb-1 text-black`}>Q1 - Q4</h3>
             <p className={`text-sm text-gray-600 mb-4`}>Key Objectives & Focus</p>
             <p className={`text-xs text-gray-700 mb-8 max-w-md`}>A breakdown of the year's primary goals and activities, distributed across quarters to ensure steady progress and alignment.</p>

             <div className="grid grid-cols-2 md:grid-cols-4 gap-y-4 gap-x-6">
                {Object.entries(quarters).map(([q, items]) => (
                  <div key={q}>
                     <div className={`text-xs font-semibold text-gray-500 mb-3 uppercase`}>{q}</div>
                     <div className="space-y-3">
                        {items.map(item => (
                            <div key={item} className={`rounded-md p-3 text-center bg-white shadow-sm border border-gray-200`}>
                                <div className={`text-xs font-medium text-gray-800 mb-1`}>{item}</div>
                                <div className={`text-xxs text-gray-500`}>Focus Area</div>
                            </div>
                        ))}
                     </div>
                  </div>
    ...
    const RoadmapTimelineCard: React.FC<{ isDark?: boolean }> = ({ isDark = true }) => (
        <CardPreview isDark={false} className={`col-span-2 md:col-span-3 p-6 bg-white`}>
            <div className={`text-xxs font-medium text-gray-400 mb-1 uppercase tracking-wider`}>Development Roadmap</div>
            <h3 className={`text-2xl font-bold mb-1 text-black`}>Roadmap</h3>
            <p className={`text-sm text-gray-600 mb-4`}>Key Milestones & Timeline</p>
            <p className={`text-xs text-gray-700 mb-10 max-w-md`}>This timeline highlights significant milestones, showing the projected path and key delivery points over the coming weeks.</p>

            <div className="relative w-full h-auto overflow-x-auto py-4 whitespace-nowrap">
                <div className={`absolute left-4 right-4 h-0.5 bg-gray-300`} style={{top: 'calc(50% - 1px)'}}></div>
                <div className="relative flex justify-between w-full px-2 md:px-4 min-w-[400px] md:min-w-full">
                    ...
                    {[{label: 'Week 1', tasks: []}, {label: 'Today', tasks: ['Kick-off Meeting', 'Requirements Finalized'] }, {label: 'Week 3', tasks: []}, {label: 'Week 4', tasks: []}, {label: 'Week 5', tasks: []}].map(({label, tasks}) => (
                         <div key={label} className="flex flex-col items-center z-10 mx-4 md:mx-0">
                            <div className={`w-3 h-3 rounded-full border-2 bg-white border-gray-600`}></div>
                             <div className={`mt-3 text-xs font-medium px-2 py-0.5 rounded ${label === 'Today' ? 'bg-gray-300 text-black' : 'text-gray-500'}`}>{label}</div>
                             {label === 'Today' && tasks.length > 0 && (
                                <div className={`mt-2 text-xxs text-gray-600 text-center leading-tight space-y-0.5`}>

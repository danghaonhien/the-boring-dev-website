...
const Pitch02ProjectPage: React.FC = () => { // Renamed component
  ...
  const createCards = useCallback((variant: 'propositions') => {
    ...
    const GraphStatsCard: React.FC<{ isDark?: boolean, data?: number[] }> = ({ isDark = false, data = [30, 60, 45, 75, 50] }) => { // Added more data points
        ...
        const barGradientTo = isDark ? 'from-gray-700' : 'from-gray-400';
        const highlightGradientFrom = 'from-blue-500';
        const highlightGradientTo = 'to-blue-600';

        return (
            <CardPreview isDark={isDark} className={`flex flex-col p-4 ${isDark ? 'bg-gray-950 border-gray-800' : 'bg-white border border-gray-200'}`}> {/* Slightly darker bg */}
